...
function Table({ rows }: { rows: Awaited<ReturnType<typeof prisma.family.findMany>> }) {return <DataTable><thead className="bg-muted"><tr><th className="p-3">Name</th><th>Email</th><th>Phone</th><th>Status</th><th>Students</th><th>Actions</th></tr></thead><tbody>{rows.map((r:any)=><tr className="border-t" key={r.id}><td className="p-3 font-medium">{r.name}</td><td>{r.email ?? '—'}</td><td>{r.phone ?? '—'}</td><td><StatusBadge status={r.status} /></td><td>{r._count.students}</td><td><form action={archiveFamily}><input type="hidden" name="id" value={r.id}/><ConfirmSubmitButton className="text-primary" message="Archive this record? This will hide it from active workflows.">Archive</ConfirmSubmitButton></form></td></tr>)}</tbody></DataTable>}

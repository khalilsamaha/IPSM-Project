...
export default async function PaymentsPage({
  ...
  const payments = await prisma.payment.findMany({
    include: { family: true, items: true },
  ...
  return (
    <AppShell session={session}>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white" href="/payments/new">
          Create payment
        </Link>
      </div>
          ...
          <tbody>
            {payments.map((payment) => (
              <tr className="border-t" key={payment.id}>
                <td className="p-3 font-medium">{payment.family.name}</td>
                <td>{formatDate(payment.paymentDate)}</td>
                <td>{payment.paymentMethod}</td>
                <td>{formatCurrency(payment.totalAmountCents)}</td>
                <td>{payment.items.length}</td>
                <td>
                  <Link className="text-primary" href={`/payments/${payment.id}`}>
                    View
                  </Link>
                </td>
              </tr>
